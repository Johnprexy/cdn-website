...
export default function Join() {
  ...
  return (
    <div>
      <PageHero eyebrow="Join CDN" title={<>Join the <span className="text-gradient-gold">Network</span></>} sub="Membership in CDN is not about events or programmes — it is about committing to a structured, consistent discipleship journey." />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
              {BENEFITS.map((b,i) => (
                <Reveal key={b.t} delay={i*0.07}>
                  <motion.div className="p-5" style={{ border:'1px solid #e0ddf0', borderLeft:'3px solid #c9a020' }} whileHover={{ background:'#fdfcf7', x: 4 }} transition={{ duration:0.2 }}>
              ...
              <AnimatePresence mode="wait">
                {submitted ? (
                  <motion.div key="success" className="text-center py-10"
                    ...
                    <motion.div style={{ fontSize:'2.5rem', marginBottom:'1rem' }} initial={{ scale:0 }} animate={{ scale:1 }} transition={{ type:'spring', bounce:0.5, delay:0.1 }}>✓</motion.div>
                    <h4 className="font-display" style={{ fontSize:'1.25rem', color:'#1a2355', marginBottom:'0.5rem' }}>Welcome to CDN</h4>
                    <p style={{ fontSize:'0.875rem', color:'#50556e' }}>Your registration has been received. Your campus chapter leader will be in touch within the week.</p>
                  </motion.div>
                ) : (
                  <motion.div key="form" className="space-y-4" exit={{ opacity:0 }}>
                    ...
                    <motion.button onClick={() => setSubmitted(true)} className="w-full py-3.5 font-bold uppercase"
                      style={{ background:'#c9a020', color:'#0d1225', fontSize:'0.875rem', letterSpacing:'0.12em', border:'none', cursor:'pointer' }}
                      whileHover={{ background:'#d4a820' }} whileTap={{ scale:0.98 }}>
                      Join the Network →
                    </motion.button>
                  </motion.div>
                )}
