        ...
        <div className="max-w-6xl mx-auto px-6 md:px-12 grid lg:grid-cols-2 gap-16 items-start">
                  ...
                  <div style={{ borderBottom: i < FAQS.length - 1 ? '1px solid #e0ddf0' : 'none' }}>
                    ...
                    >
                      ...
                      <motion.span animate={{ rotate: openFaq === i ? 180 : 0 }} transition={{ duration: 0.25 }}>
                        <ChevronDown size={16} style={{ color: '#c9a020', flexShrink: 0 }} />
                      </motion.span>
                    </motion.button>
                    <AnimatePresence>
                      {openFaq === i && (
          ...
          <Reveal direction="right" delay={0.1}>
            ...
            <AnimatePresence mode="wait">
              {sent ? (
                <motion.div key="success" className="text-center py-16" style={{ background: '#f2f0f8', border: '1px solid #e0ddf0' }}
                ...
                <motion.div key="form" className="space-y-4" initial={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <motion.button onClick={() => setSent(true)} className="w-full py-3.5 font-bold uppercase"
                    style={{ background: '#1e2d6b', color: 'white', fontSize: '0.875rem', letterSpacing: '0.12em', border: 'none', cursor: 'pointer' }}
                    whileHover={{ background: '#2a3d8f' }} whileTap={{ scale: 0.98 }}>
                    Send Message →
                  </motion.button>
                </motion.div>
              )}
